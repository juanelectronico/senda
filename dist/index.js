...
function validarSAT(cer, key, pass) {
    ...
    if (!base64Regex.test(cer))
        errors.push('El .cer no tiene formato Base64 válido');
    if (!base64Regex.test(key))
        errors.push('El .key no tiene formato Base64 válido');
    if (cer.length < 100)
        errors.push('El .cer parece demasiado corto');
    if (key.length < 100)
        errors.push('El .key parece demasiado corto');
    if (pass.length < 4)
