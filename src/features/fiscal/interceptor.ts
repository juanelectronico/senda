...
export class FiscalInterceptor {
  ...
  constructor() {
    ...
    this.fiscalFlow = new FiscalFlow(
      new ConversationStateManager(),
      new FiscalValidator(),
      new MerchantNotifier(),
  ...
  async intercept(message: string, userId: string): Promise<string | null> {
    try {
  ...
  private isInvoiceIntent(message: string): boolean {
